...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() {
    ...
    var a = Math.PI / 180,
        ...
        i = "MorphSVGPlugin",
        j = "greensock.com",
        k = "/requires-membership/",
        l = function(a) {
            for (var b = -1 !== (window ? window.location.href : "").indexOf("mtn") && -1 !== a.indexOf("localhost"), c = [j, "mtn", "codepen.io", "codepen.dev", "css-tricks.com", "cdpn.io", "gannon.tv", "codecanyon.net", "themeforest.net", "cerebrax.co.uk", "tympanus.net", "tweenmax.com", "tweenlite.com", "plnkr.co", "hotjar.com", "jsfiddle.net"], d = c.length; --d > -1;)
                if (-1 !== a.indexOf(c[d])) return !0;
            return b && window && window.console && console.log("WARNING: a special version of " + i + " is running locally, but it will not work on a live domain because it is a membership benefit of Club GreenSock. Please sign up at http://greensock.com/club/ and then download the 'real' version from your GreenSock account which has no such limitations. The file you're using was likely downloaded from elsewhere on the web and is restricted to local use or on sites like codepen.io."), b
...
$(window).on('resize',function(){
	window.w.height = $(window).height();
	window.w.width = $(window).width();
...
function SingleAnimation(){
	...
	this.animate = function( duration, update, complete ){
		_this.animating = true;
		TweenMax.to( anim, duration || 0.5, { x: 0, onUpdate: function(){
			if (update) update( this.progress() );
		}, onComplete: function(){
