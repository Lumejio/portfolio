...
$menuItems.on('click',function(e){
	...
	var pos = $('.nav-anchor').eq( index ).position().top-47;
	$('html, body').stop().animate({ scrollTop: pos }, 500, function(){
		menuScrolling = false;
		$(window).triggerHandler('scroll');
	});
	$menuItems.removeClass('active').eq( index ).addClass('active');
	e.preventDefault();
});
...
function scrollUpdate(){
	...
	if (sct < 150) {
		var pos = -sct*0.7;
		$menu.css('transform','translateY('+pos+'px)');
	}
	if (sct < 335 && fixedMenu) {
		fixedMenu = false;
		$menu.removeClass('fixed');
	} else if (sct >= 335 && !fixedMenu) {
		fixedMenu = true;
		$menu.addClass('fixed');
	}
	...
	var index = -1;
	var pos = sct+innerHeight*0.35;
	$('.nav-anchor').each(function(i){
		if (pos > $(this).position().top) index = i;
	});
	/*if (prevIndex != index) {
		prevIndex = index;
		_menu.$innerNav.children().removeClass('active').eq( index ).addClass('active');
		_menu.$outerNav.children().removeClass('active').eq( index ).addClass('active');
	}*/
	...
	$('.anim-anchor').each(function(i){
		// if (pos < $(this).position().top) return;
		var top = $(this).getOffsetTop();
		var offset = $(this).data('scroll');
		if (offset) top -= offset;
		if (pos < top) return;
		
		$(this).removeClass('anim-anchor');
		var $anims = $(this).find('.anim').add( this ).filter('.anim');
		$anims.each(function(i){
			var $elem = $(this);
			var data = $elem.data('anim').split(';');
			var type = data[0];
			var delay = parseInt(data[1]) || 100;

			var animFunc = type in Animations ? Animations[type] : Animations['default'];
			setTimeout(function(){
...
var AnimationsPre = {
	gooey_features: function( initDelay ){
		var $items = this.find('.features-item').addClass('anim').each(function(i){
			var value = 'gooey_positions;'+(initDelay+(3-i)*160);
			$(this).attr('data-anim', value );
		});
		var offsetDelay = initDelay+$items.length*90+850;
		this.find('img').addClass('anim').attr('data-anim', 'f;'+offsetDelay );
		this.find('h6, p, .button').addClass('anim').attr('data-anim', 'fb;'+offsetDelay );

		$(this).addClass('anim').attr('data-anim','gooey_filter;'+(offsetDelay-20) );
	},
	gooey_complex: function( initDelay ){
		var $items = this.find('.complex-item').addClass('anim').each(function(i){
			var value = 'gooey_positions;'+(initDelay+(2-i)*160);
			$(this).attr('data-anim', value );
		});
		var offsetDelay = initDelay+$items.length*90+850;
		this.find('img').addClass('anim').attr('data-anim', 'f;'+offsetDelay );
		this.find('h6, li').addClass('anim').attr('data-anim', 'fb;'+offsetDelay );

		$(this).addClass('anim').attr('data-anim','gooey_filter;'+(offsetDelay-20) );
	},
	gooey_main: function( initDelay ){
		var $mainItems = this.find('.main-item').addClass('anim').each(function(i){
			var value = 'gooey_positions;'+(initDelay+i*90);
			$(this).attr('data-anim', value );
		});
		var offsetDelay = initDelay+$mainItems.length*90+850;
		this.find('img').addClass('anim').attr('data-anim', 'f;'+offsetDelay );
		this.find('p').addClass('anim').attr('data-anim', 'fb;'+offsetDelay );

		$(this).addClass('anim').attr('data-anim','gooey_filter;'+(offsetDelay-20) );
	},

	state: function( initDelay ){
		this.find('.number').addClass('anim').each(function(i){
			var value = 'fr;'+(initDelay+i*350);
			$(this).attr('data-anim', value );
		});
		this.find('p').addClass('anim').each(function(i){
			var value = 'fr;'+(initDelay+i*350+150);
			$(this).attr('data-anim', value );
		});
	},
	photo: function( initDelay ){
		this.children().addClass('anim').each(function(i){
			var value = 'fr;'+(initDelay+i*150);
			$(this).attr('data-anim', value );
		});
	},
	lists: function( initDelay ){
		this.children().addClass('anim').each(function(i){
			var value = 'fb;'+(initDelay+i*150);
			$(this).attr('data-anim', value );
		});
	}
}
$('.anim-pre').each(function(i){
	var data = $(this).data('pre').split(';');
	var type = data[0];
	var delay = parseInt(data[1]) || 100;

	if (type in AnimationsPre) {
		AnimationsPre[type].call( $(this), delay );
	}
});
...
$(document).on('click','.send',function(e){
	...
	// if ($form.find('input[name="phone"]').val() == '') return alert('Укажите номер телефона');
